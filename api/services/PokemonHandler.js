...
exports.computeCloneHash = pkmn => {
  ...
  buf.writeUInt8(pkmn.natureId, 38); // 1 byte
  let compactIvs = 0;
  [pkmn.ivHp, pkmn.ivAtk, pkmn.ivDef, pkmn.ivSpe, pkmn.ivSpAtk, pkmn.ivSpDef].forEach(value => {
    compactIvs = compactIvs << 5 | value;
  });
  buf.writeUInt32LE(compactIvs >>> 0, 39); // (5 bits per IV) * (6 IVs) + (2 blank bits) = 32 bits = 4 bytes
    // Total: 43 bytes (initialized in buffer above)
